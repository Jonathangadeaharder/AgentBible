...
def create_skill_files(skills_dir, languages):
    ...
    lang_config = {
        'csharp': {
            ...
            'files': ['clean-code.md', 'patterns.md', 'testing.md', 'static-analysis.md', 'parallel.md', 'ipc.md']
        },
        'cpp': {
            'folder': 'skills/cpp',
    ...
    for lang in languages:
        ...
        if skill_md.exists():
            ...
            print(f"  ✓ skills/{lang_key}/ (SKILL.md + {len(config['files'])} topic files)")
            count += 1
...
def create_global_claude_md(target_file):
    ...
    sections.append("> Universal coding standards. These rules apply to all languages and projects.\n")
